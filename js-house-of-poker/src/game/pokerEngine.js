...
const INVITE_RECIPIENT_BLUEPRINTS = [
  ...
  {
    description: 'Free-play regular surfaced from friends, feed posts, and shared table history.',
  ...
  {
    description: 'Private-table regular who often joins through username lookups.',
    handle: 'late-reg',
    id: 'friend-list-2',
    label: 'Late Reg',
    source: 'friend-list',
  ...
  {
    description: 'Fast-table regular available from the friend list and player directory.',
    handle: 'stack-sprinter',
    id: 'friend-list-3',
    label: 'Stack Sprinter',
    source: 'friend-list',
  },
  {
    description: 'Social poster who can be invited from feed cards or username search.',
    handle: 'pocket-poet',
    id: 'friend-list-4',
    label: 'Pocket Poet',
    source: 'friend-list',
...
const MOCK_PLAYER_STATUS_BY_NAME = Object.freeze({
  artie: 'up_and_coming',
  poker4ever: 'high_roller',
  thor: 'high_roller',
  ullii67: 'shark',
  vossell: 'shark',
});
const MOCK_STATUS_STATE_BY_TIER = Object.freeze({
  high_roller: Object.freeze({
    netChipBalance: BIG_BLIND * 18,
    recentHands: 6,
    recentScore: 86,
    reputation: 84,
  }),
  shark: Object.freeze({
    netChipBalance: BIG_BLIND * 30,
    recentHands: 8,
    recentScore: 132,
    reputation: 96,
    sharkWins: 2,
  ...
  up_and_coming: Object.freeze({
    netChipBalance: BIG_BLIND * 8,
    recentHands: 5,
    recentScore: 56,
    reputation: 72,
  }),
...
function normalizeMockPlayerName(name) {
  return typeof name === 'string' ? name.trim().toLowerCase() : '';
}

function createMockPlayerStatusHistory(fixture, occurredAt) {
  const recentHands = Math.max(1, Math.round(fixture.recentHands));
  const scoreBase = Math.floor((fixture.recentScore / recentHands) * 10) / 10;
  const chipBase = Math.floor(fixture.netChipBalance / recentHands);

  return Array.from({ length: recentHands }, (_, index) => {
    const isLastEntry = index === recentHands - 1;
    return {
      beatSharks: isLastEntry ? fixture.sharkWins ?? 0 : 0,
      fieldStrength: 3,
      netBigBlinds: isLastEntry
        ? Number(((fixture.netChipBalance - chipBase * index) / BIG_BLIND).toFixed(2))
        : Number((chipBase / BIG_BLIND).toFixed(2)),
      netChips: isLastEntry ? fixture.netChipBalance - chipBase * index : chipBase,
      occurredAt: occurredAt - (recentHands - index - 1) * 60_000,
      profitableHand: true,
      scoreDelta: isLastEntry
        ? Number((fixture.recentScore - scoreBase * index).toFixed(1))
        : scoreBase,
      strongTableWin: isLastEntry ? (fixture.strongTableWins ?? 0) > 0 : false,
      strongerPlayersBeaten: isLastEntry ? fixture.strongTableWins ?? 0 : 0,
    };
  });
}

function createMockPlayerStatusState(name) {
  const statusTier = MOCK_PLAYER_STATUS_BY_NAME[normalizeMockPlayerName(name)];
  const fixture = statusTier ? MOCK_STATUS_STATE_BY_TIER[statusTier] : null;

  if (!fixture) {
    return createInitialPlayerStatusState();
  }

  const occurredAt = Date.now();
  return sanitizePlayerStatusState({
    ...createInitialPlayerStatusState(),
    ...fixture,
    history: createMockPlayerStatusHistory(fixture, occurredAt),
    lastUpdatedAt: occurredAt,
    playerStatus: statusTierToPlayerStatus(statusTier),
    statusTier,
    statusUpdatedAt: occurredAt,
  });
}

function createPlayer(socketId, name, options = {}) {
  ...
  return {
    ...
    socketId,
    statusState: options.seedMockStatuses
      ? createMockPlayerStatusState(playerName)
      : createInitialPlayerStatusState(),
  };
}
...
function applyStatusResults(room, payouts) {
  ...
  room.players.forEach((player) => {
    if (nextStatusStates[player.id]) {
      player.statusState = nextStatusStates[player.id];
    }
  });
}
...
function joinRoom(room, socketId, name, options = {}) {
...
function buildRoomState(room, playerId) {
  ...
  const players = room.players.map((player) => {
    ...
    return {
      ...
      statusMomentum: statusState.statusMomentum,
      statusScore: statusState.statusScore,
      statusSnapshot: toPublicPlayerStatusSnapshot(statusState),
      statusTier: statusState.statusTier,
      statusUpdatedAt: statusState.statusUpdatedAt,
